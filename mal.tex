...
In the current design, the master launches one goroutinea - a SDL window where the user enters keyboard input and another goroutine multicasting new goroutines to the slaves. These goroutines use a channel and one way communication to transfer packed states from the goroutine receiving user input to the goroutine that actually sends the states. 

The channel is unbuffered, which means that it has a s
